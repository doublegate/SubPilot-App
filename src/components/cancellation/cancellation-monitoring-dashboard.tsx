...
export function CancellationMonitoringDashboard() {
  const [selectedTimeframe, setSelectedTimeframe] = useState<'day' | 'week' | 'month' | 'quarter' | 'year'>('month');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw className={`w-4 h-4 mr-2 ${(loadingHealth || loadingAnalytics) ? 'animate-spin' : ''}`} />
            Refresh
          </Button>
          <Button
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {systemHealth?.methods && Object.entries(systemHealth.methods).map(([method, methodData]) => {
                    ...
                    <div key={method} className="p-4 border rounded-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{data.recentRequests}</span>
                        </div>
          ...
          {systemHealth?.recommendations && systemHealth.recommendations.length > 0 && (
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {systemHealth.recommendations.map((recommendation: any, index: number) => (
                    <Alert key={index}>
                      <AlertCircle className="w-4 h-4" />
                      <AlertDescription>{String(recommendation)}</AlertDescription>
                    </Alert>
                  ))}
                </div>
              </CardContent>
            ...
            <>
                ...
                <CardContent>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold">
                        {formatDuration(systemHealth.detailed.performanceMetrics.p50ResponseTime)}
                      </div>
                      <p className="text-sm text-muted-foreground">P50 (Median)</p>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold">
                        {formatDuration(systemHealth.detailed.performanceMetrics.p95ResponseTime)}
                      </div>
                      <p className="text-sm text-muted-foreground">P95</p>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold">
                        {formatDuration(systemHealth.detailed.performanceMetrics.p99ResponseTime)}
                      </div>
              ...
              {Object.keys(systemHealth.detailed.errorBreakdown).length > 0 && (
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {Object.entries(systemHealth.detailed.errorBreakdown).map(([error, count]) => (
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {systemHealth.detailed.uptimeMetrics.lastDowntime 
                          ? new Date(systemHealth.detailed.uptimeMetrics.lastDowntime).toLocaleString()
                          : 'None recorded'
                        }
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {systemHealth.detailed.uptimeMetrics.plannedMaintenance 
                          ? new Date(systemHealth.detailed.uptimeMetrics.plannedMaintenance).toLocaleString()
                          : 'None scheduled'
                        }
                      </span>
                    </div>
        ...
        <TabsContent value="analytics" className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {(['day', 'week', 'month', 'quarter', 'year'] as const).map((timeframe) => (
                <Button
                  key={timeframe}
            ...
            return (
              <>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-600">{analyticsData.summary.completed}</div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-red-600">{analyticsData.summary.failed}</div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-orange-600">{analyticsData.summary.pending}</div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{analyticsData.summary.successRate}%</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {Object.entries(analyticsData.methodBreakdown).map(([method, count]) => (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="w-20 bg-gray-200 rounded-full h-2">
                              <div
                                ...
                                style={{ width: `${(count as number / analyticsData.summary.total) * 100}%` }}
                              />
                            </div>
                          </div>
                        </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {Object.entries(analyticsData.successRates).map(([method, rate]) => (
                        <div key={method} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            {getMethodIcon(method)}
                            <span className="capitalize">{method}</span>
              ...
              {analyticsData.providerAnalytics.length > 0 && (
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {analyticsData.providerAnalytics.map((provider: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                          <div className="flex-1">
                            <div className="font-medium">{provider.provider}</div>
                            <div className="text-sm text-muted-foreground">
                              {provider.totalAttempts} attempts • {provider.averageCompletionTime}min avg
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-medium">{provider.successRate}%</span>
                            <Progress value={provider.successRate} className="w-20 h-2" />
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <CardContent>
              <div className="h-64 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  ...
                  <p>Performance trend visualization would be implemented here</p>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-sm font-medium">{systemHealth.system.cpu}%</span>
                      </div>
                      <Progress value={systemHealth.system.cpu} className="h-2" />
                    </div>
                    <div>
                      <div className="flex justify-between mb-1">
                        <span className="text-sm">Memory Usage</span>
                        <span className="text-sm font-medium">{systemHealth.system.memory}%</span>
                      </div>
                      <Progress value={systemHealth.system.memory} className="h-2" />
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">Active Connections</span>
                      <span className="text-sm font-medium">{systemHealth.system.activeConnections}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
