...
export async function GET() {
  ...
  if (!v5Check.hasAuthSecret && !v5Check.hasNextAuthSecret) {
    recommendations.push(
      'Missing AUTH_SECRET or NEXTAUTH_SECRET - this is required for production'
    );
  }

  if (!v5Check.hasAuthUrl && !v5Check.hasNextAuthUrl) {
    recommendations.push(
      'Missing AUTH_URL or NEXTAUTH_URL - this is required for OAuth callbacks'
    );
  }
  ...
  return NextResponse.json({
    ...
    recommendations,
    message:
      'This endpoint tests OAuth provider configuration directly without env validation',
  });
}
