...
export async function GET() {
  ...
  try {
    ...
    diagnostics.envValidation.envError = error instanceof Error ? error.message : String(error);
  }
  ...
  try {
    ...
    diagnostics.providers.error = error instanceof Error ? error.message : String(error);
  }
  ...
  try {
    ...
    diagnostics.authConfig.error = error instanceof Error ? error.message : String(error);
  }
  ...
  const checkEndpoint = async (path: string) => {
    try {
      ...
      return {
        ...
        ok: response.ok,
        data: response.ok ? await response.text().then(text => {
          try {
  ...
  if (diagnostics.rawEnvironment.GOOGLE_CLIENT_ID.isEmpty || 
      diagnostics.rawEnvironment.GOOGLE_CLIENT_SECRET.isEmpty) {
    diagnostics.recommendations.push('Google OAuth credentials appear to be empty strings. Check Vercel environment variables.');
  }
  
  if (diagnostics.rawEnvironment.GITHUB_CLIENT_ID.isEmpty || 
      diagnostics.rawEnvironment.GITHUB_CLIENT_SECRET.isEmpty) {
    diagnostics.recommendations.push('GitHub OAuth credentials appear to be empty strings. Check Vercel environment variables.');
  }
  
  if (!diagnostics.rawEnvironment.NEXTAUTH_URL && !process.env.VERCEL_URL) {
    diagnostics.recommendations.push('NEXTAUTH_URL is not set and VERCEL_URL is not available. OAuth callbacks may fail.');
  }
  
  if (diagnostics.envValidation.validatedEnv && 
      !diagnostics.envValidation.validatedEnv.GOOGLE_CLIENT_ID && 
      diagnostics.rawEnvironment.GOOGLE_CLIENT_ID.exists) {
    diagnostics.recommendations.push('Environment validation is stripping out OAuth credentials. Check for empty strings or validation rules.');
  }
