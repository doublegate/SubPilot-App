...
export async function GET() {
  ...
  if (!results.v5Naming.AUTH_SECRET && !results.currentNaming.NEXTAUTH_SECRET) {
    results.recommendation =
      'CRITICAL: Missing AUTH_SECRET (v5) or NEXTAUTH_SECRET (v4). Add AUTH_SECRET to Vercel.';
  } else if (
    !results.v5Naming.AUTH_URL &&
    !results.currentNaming.NEXTAUTH_URL
  ) {
    results.recommendation =
      'CRITICAL: Missing AUTH_URL (v5) or NEXTAUTH_URL (v4). Add AUTH_URL to Vercel.';
  } else if (
    !results.oauthProviders.AUTH_GOOGLE_ID &&
    !results.oauthProviders.GOOGLE_CLIENT_ID
  ) {
    results.recommendation =
      'OAuth providers use AUTH_[PROVIDER]_ID pattern in v5. Try AUTH_GOOGLE_ID and AUTH_GOOGLE_SECRET.';
  } else {
    results.recommendation =
      'Environment variables appear to be set. Check the values are not empty strings.';
  }
