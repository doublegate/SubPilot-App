    ...
    const debugInfo = {
      ...
      recommendations: [
        !envVars.AUTH_URL && !envVars.NEXTAUTH_URL
          ? 'Set AUTH_URL or NEXTAUTH_URL to your deployment URL'
          : null,
        !sessionToken && session
          ? 'Session exists but no session cookie - possible cookie issue'
          : null,
        sessionToken && !session
          ? 'Session cookie exists but no session - possible secret mismatch'
          : null,
