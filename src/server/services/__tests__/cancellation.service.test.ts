...
describe("CancellationService", () => {
  ...
  describe("initiateCancellation", () => {
    ...
    it("should initiate cancellation successfully", async () => {
      ...
      vi.mocked(mockPrismaClient.subscription.findFirst).mockResolvedValueOnce(mockSubscription as any);
      vi.mocked(mockPrismaClient.cancellationProvider.findFirst).mockResolvedValueOnce(mockProvider as any);
      vi.mocked(mockPrismaClient.cancellationRequest.create).mockResolvedValueOnce(mockRequest as any);
      vi.mocked(mockPrismaClient.cancellationLog.create).mockResolvedValueOnce({} as any);

      // Mock the processCancellation method
      const processSpy = vi.spyOn(service, "processCancellation").mockResolvedValueOnce(mockResult as any);
    ...
    it("should throw error if subscription not found", async () => {
      vi.mocked(mockPrismaClient.subscription.findFirst).mockResolvedValueOnce(null);
    ...
    it("should throw error if subscription already cancelled", async () => {
      ...
      vi.mocked(mockPrismaClient.subscription.findFirst).mockResolvedValueOnce(cancelledSubscription as any);
  ...
  describe("getCancellationStatus", () => {
    it("should return cancellation status", async () => {
      ...
      vi.mocked(mockPrismaClient.cancellationRequest.findFirst).mockResolvedValueOnce(mockRequest as any);
    ...
    it("should throw error if request not found", async () => {
      vi.mocked(mockPrismaClient.cancellationRequest.findFirst).mockResolvedValueOnce(null);
  ...
  describe("retryCancellation", () => {
    it("should retry failed cancellation", async () => {
      ...
      vi.mocked(mockPrismaClient.cancellationRequest.findFirst).mockResolvedValueOnce(mockFailedRequest as any);
      vi.mocked(mockPrismaClient.cancellationRequest.update).mockResolvedValueOnce({} as any);

      const processSpy = vi.spyOn(service, "processCancellation").mockResolvedValueOnce({
        requestId: "req123",
        status: "processing",
  ...
  describe("confirmManualCancellation", () => {
    it("should confirm manual cancellation", async () => {
      ...
      vi.mocked(mockPrismaClient.cancellationRequest.findFirst).mockResolvedValueOnce(mockRequest as any);
      vi.mocked(mockPrismaClient.cancellationRequest.update).mockResolvedValueOnce({
        ...mockRequest,
        status: "completed",
        ...confirmationData,
      } as any);
      vi.mocked(mockPrismaClient.subscription.update).mockResolvedValueOnce({} as any);
      vi.mocked(mockPrismaClient.cancellationLog.create).mockResolvedValueOnce({} as any);

      const result = await service.confirmManualCancellation("user123", "req123", confirmationData);
  ...
  describe("getCancellationHistory", () => {
    it("should return user cancellation history", async () => {
      ...
      vi.mocked(mockPrismaClient.cancellationRequest.findMany).mockResolvedValueOnce(mockRequests as any);
