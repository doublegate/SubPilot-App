...
export const UnifiedCancellationRequestInput = z.object({
  ...
  preferredMethod: z.enum(['auto', 'api', 'automation', 'manual']).optional().default('auto'),
...
export type UnifiedCancellationRequest = z.infer<typeof UnifiedCancellationRequestInput>;
...
export interface UnifiedCancellationResult {
  ...
  requestId: string;
  status: 'initiated' | 'routing' | 'processing' | 'completed' | 'failed' | 'requires_manual' | 'scheduled';
...
export class UnifiedCancellationOrchestratorEnhancedService {
  ...
  private providerCapabilities = new Map<string, { capability: ProviderCapability; expires: Date }>();
  ...
  async initiateCancellation(
    ...
    try {
      ...
      const subscription = await this.validateSubscriptionOwnership(userId, input.subscriptionId);
      
      // Check if cancellation is allowed
      await this.validateCancellationEligibility(userId, input.subscriptionId, subscription);
      
      // Assess provider capabilities
      const capabilities = await this.assessProviderCapabilities(subscription.name);
  ...
  private async executeMethod(
    ...
    const baseResult = {
      ...
      metadata: {
        ...
        realTimeUpdatesEnabled: input.userPreferences?.notificationPreferences?.realTime ?? true,
      },
      tracking: {
    ...
    switch (method) {
      case 'api':
        return await this.executeApiMethod(userId, subscription, input, baseResult, capabilities);
      
      case 'automation':
        return await this.executeAutomationMethod(userId, subscription, input, baseResult, capabilities);
      
      case 'manual':
        return await this.executeManualMethod(userId, subscription, input, baseResult, capabilities);
  ...
  private async executeApiMethod(
    ...
    try {
      const apiResult = await this.cancellationService.initiateCancellation(userId, {
      ...
      throw new Error(`API cancellation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async executeAutomationMethod(
    ...
    try {
      const automationResult = await this.eventDrivenService.initiateCancellation(userId, {
        subscriptionId: input.subscriptionId,
        priority: input.priority,
        notes: input.reason,
      ...
      throw new Error(`Automation cancellation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async executeManualMethod(
    ...
    try {
      const manualResult = await this.lightweightService.provideCancellationInstructions(userId, {
        subscriptionId: input.subscriptionId,
        notes: input.reason,
      });
      ...
      throw new Error(`Manual cancellation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private determineOptimalMethod(
    ...
    if (capabilities.supportsAutomation && (
      ...
      capabilities.automationSuccessRate > 0.70
    )) {
      return 'automation';
    }
  ...
  private buildFallbackChain(
    ...
    const methods: Array<'api' | 'automation' | 'manual'> = ['api', 'automation', 'manual'];
    
    // Add other supported methods in order of preference
    for (const method of methods) {
      if (method !== primaryMethod && this.isMethodSupported(method, capabilities)) {
        chain.push(method);
      }
    }
   ...
   */
  private async assessProviderCapabilities(subscriptionName: string): Promise<ProviderCapability> {
    ...
    if (provider) {
      ...
      capability = {
        ...
        supportsManual: true,
        apiSuccessRate: provider.type === 'api' ? parseFloat(provider.successRate.toString()) : 0,
        automationSuccessRate: provider.type === 'web_automation' ? parseFloat(provider.successRate.toString()) : 0,
        manualSuccessRate: 0.95, // Manual instructions are generally reliable
        apiEstimatedTime: provider.type === 'api' ? (provider.averageTime || 5) : 0,
        automationEstimatedTime: provider.type === 'web_automation' ? (provider.averageTime || 15) : 0,
        ...
        requiresHumanIntervention: provider.requires2FA || provider.requiresRetention,
   ...
   */
  private generateHeuristicCapability(subscriptionName: string): ProviderCapability {
    const name = subscriptionName.toLowerCase();
    
    // Heuristics based on common patterns
    const isStreamingService = /netflix|hulu|disney|prime|spotify|apple|youtube/.test(name);
   ...
   * Validate subscription ownership
   */
  private async validateSubscriptionOwnership(userId: string, subscriptionId: string) {
  ...
  private async validateCancellationEligibility(
    ...
    if (existingRequest) {
      throw new TRPCError({
        code: 'CONFLICT',
        message: 'A cancellation request is already in progress for this subscription',
  ...
  private setupEventListeners(): void {
    ...
    console.log('[UnifiedCancellationOrchestratorEnhanced] Event listeners initialized');
  }
  ...
  private async initializeCapabilityCache(): Promise<void> {
    try {
      ...
      console.log(`[UnifiedCancellationOrchestratorEnhanced] Initialized capabilities for ${providers.length} providers`);
    } catch (error) {
      console.error('[UnifiedCancellationOrchestratorEnhanced] Failed to initialize capability cache:', error);
    }
  }
   ...
   * Register active orchestration for real-time tracking
   */
  private registerActiveOrchestration(orchestrationId: string, data: any): void {
    ...
    if (orchestration) {
      ...
      orchestration.callbacks.forEach(callback => {
        try {
          ...
          console.error('[UnifiedCancellationOrchestratorEnhanced] Error in update callback:', error);
        }
      });
  ...
  private async handleServiceCompletion(data: any): Promise<void> {
    ...
    console.log('[UnifiedCancellationOrchestratorEnhanced] Service completion:', data);
  }
  ...
  private async handleServiceProgress(data: any): Promise<void> {
    ...
    console.log('[UnifiedCancellationOrchestratorEnhanced] Service progress:', data);
  }
  ...
  private async handleServiceFailure(data: any): Promise<void> {
    ...
    console.log('[UnifiedCancellationOrchestratorEnhanced] Service failure:', data);
  }
  ...
  private async logOrchestrationActivity(
    ...
    try {
      ...
      console.error('[UnifiedCancellationOrchestratorEnhanced] Failed to log activity:', error);
    }
  }
  ...
  async getUnifiedAnalytics(
    ...
    const pending = requests.filter(r => ['pending', 'processing', 'scheduled'].includes(r.status)).length;

    // Method breakdown
    const methodCounts = requests.reduce((acc, r) => {
      const method = r.method === 'manual' ? 'manual' : 
                   r.method === 'web_automation' ? 'automation' : r.method;
      acc[method] = (acc[method] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    // Success rates by method
    const successRates = Object.keys(methodCounts).reduce((acc, method) => {
      const methodRequests = requests.filter(r => {
        const requestMethod = r.method === 'manual' ? 'manual' : 
                             r.method === 'web_automation' ? 'automation' : r.method;
        return requestMethod === method;
      });
      const successful = methodRequests.filter(r => r.status === 'completed').length;
      acc[method] = methodRequests.length > 0 ? Math.round((successful / methodRequests.length) * 100) : 0;
      return acc;
    }, {} as Record<string, number>);
    ...
    for (const request of requests) {
      ...
      const stats = providerStats.get(providerName) || { total: 0, successful: 0, totalTime: 0 };
      ...
      if (request.completedAt && request.createdAt) {
        stats.totalTime += request.completedAt.getTime() - request.createdAt.getTime();
      }
    ...
    const providerAnalytics = Array.from(providerStats.entries()).map(([provider, stats]) => ({
      ...
      averageCompletionTime: stats.successful > 0 ? Math.round(stats.totalTime / stats.successful / 1000 / 60) : 0, // minutes
    }));
    ...
    const trends = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayRequests = requests.filter(r => 
        r.createdAt.toDateString() === date.toDateString()
      );
      return {
