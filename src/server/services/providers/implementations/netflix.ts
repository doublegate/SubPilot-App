    ...
    try {
      ...
      if (scenario < 0.7) {
        ...
        return {
          ...
          error: {
            code: "AUTH_FAILED",
            message: "Invalid Netflix credentials. Please reconnect your Netflix account.",
          },
        };
      } else {
        // General failure
        return {
          ...
          error: {
            code: "PROVIDER_ERROR",
            message: "Netflix cancellation is temporarily unavailable. Please try again later or cancel manually.",
          },
        };
      }
    } catch (error) {
