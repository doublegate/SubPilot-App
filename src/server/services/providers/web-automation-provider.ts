...
export class WebAutomationProvider implements CancellationProvider {
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          message:
            error instanceof Error ? error.message : 'Web automation failed',
          details: error,
        },
      };
    } finally {
  ...
  private parseProviderConfig(provider: any): WebAutomationConfig {
    ...
    if (provider.automationScript?.steps) {
      config.navigationSteps = provider.automationScript
        .steps as NavigationStep[];
    }
    ...
    try {
      ...
      switch (step.action) {
        ...
        case 'waitForSelector':
          if (!step.selector)
            throw new Error('No selector provided for waitForSelector');
  ...
  private async detectAntiAutomation(page: Page): Promise<{
    ...
    try {
      ...
      if (
        rateLimitText?.toLowerCase().includes('rate limit') ||
