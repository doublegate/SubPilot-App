...
class AccessibilityAuditor {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('alt=""') && !line.includes('aria-hidden')) {
        this.addIssue({
          ...
          recommendation: 'Add aria-hidden="true" for decorative images with empty alt',
          wcagGuideline: 'WCAG 2.1 AA - 1.1.1 Non-text Content',
        });
      }
    });
  ...
  private checkButtons(filePath: string, lines: string[]): void {
    lines.forEach((line, index) => {
      ...
      if (line.includes('<button') && 
        ...
        const hasTextContent = lines.slice(index, index + 3).some(nextLine => 
        ...
        if (!hasTextContent) {
          this.addIssue({
            ...
            recommendation: 'Add aria-label, aria-labelledby, or text content to buttons',
      ...
      if (line.includes('size="icon"') && !line.includes('aria-label')) {
        ...
        const hasScreenReaderText = lines.slice(index, index + 5).some(nextLine =>
          nextLine.includes('sr-only')
        );
      ...
      const shadcnComponents = [
        'Button', 'DropdownMenuItem', 'MenuItem', 'TabsTrigger', 'AccordionTrigger', 
        'SelectItem', 'CommandItem', 'Card', 'Badge', 'TableRow', 'TableCell',
        'NavigationMenuItem', 'ContextMenuItem', 'AlertDialogTrigger', 'DialogTrigger',
        'TooltipTrigger', 'PopoverTrigger', 'SheetTrigger', 'CalendarDay'
      ];
      
      const isInteractiveComponent = shadcnComponents.some(component => 
        line.includes(`<${component}`) || line.includes(`${component}`) && line.includes('onClick')
      );
  ...
  private checkForms(filePath: string, lines: string[]): void {
    lines.forEach((line, index) => {
      ...
      if (line.includes('<input') && 
        ...
        const hasLabel = lines.slice(Math.max(0, index - 5), index + 5).some(nearLine =>
          nearLine.includes('<label') || nearLine.includes('htmlFor') || nearLine.includes('aria-label')
        );
  ...
  private checkColors(filePath: string, lines: string[]): void {
    lines.forEach((line, index) => {
      ...
      const hasLightText = line.includes('text-gray-300') || line.includes('text-gray-400');
      const hasLightBackground = line.includes('bg-white') || line.includes('bg-gray-50');
      ...
      if (hasLightText && hasLightBackground) {
        this.addIssue({
          ...
          issue: 'Light text on light background may not meet contrast requirements',
          recommendation: 'Use darker text colors (gray-600 or darker) on light backgrounds',
  ...
  private checkKeyboardNavigation(filePath: string, lines: string[]): void {
    lines.forEach((line, index) => {
      ...
      if (line.includes('tabIndex') && line.includes('-1') && !line.includes('aria-hidden')) {
        this.addIssue({
          file: filePath,
          line: index + 1,
          type: 'warning',
          ...
          recommendation: 'Ensure focus management is intentional and accessible',
          wcagGuideline: 'WCAG 2.1 AA - 2.1.1 Keyboard',
        });
      }

      // Check for custom keyboard handlers
      if (line.includes('onKeyDown') && !line.includes('Enter') && !line.includes('Space')) {
        this.addIssue({
          file: filePath,
          line: index + 1,
          type: 'info',
        ...
        if (match) {
          ...
          if (!hasReferencedId) {
            this.addIssue({
              ...
              recommendation: 'Ensure aria-describedby references valid element IDs',
              wcagGuideline: 'WCAG 2.1 AA - 4.1.2 Name, Role, Value',
            });
          }
        }
  ...
  private checkSemanticStructure(filePath: string, lines: string[]): void {
    ...
    lines.forEach((line, index) => {
      if (line.includes('<li') && line.includes('>')) {
        ...
        const hasListContainer = lines.slice(Math.max(0, index - 10), index + 1).some(prevLine => 
          prevLine.includes('<ul') || prevLine.includes('<ol')
        );
        
        // Also check if we're in a list context by looking at indentation/structure
        const currentIndent = line.search(/\S/);
        const hasProperNesting = lines.slice(Math.max(0, index - 10), index).some(prevLine => {
          const prevIndent = prevLine.search(/\S/);
          return (prevLine.includes('<ul') || prevLine.includes('<ol')) && prevIndent < currentIndent;
        });
  ...
  private calculateScore(stats: any): number {
    ...
    const totalPenalty = Math.min(maxPenalty, errorPenalty + warningPenalty + infoPenalty);
