  ...
  try {
    ...
    for (const user of users) {
      ...
      if (user.subscriptions.length > 0) {
        ...
        console.log(
          `  - Inactive Subscriptions: ${user.subscriptions.length - active.length}`
        );
      }
    }
    ...
    const issues = {
      noAmount: allSubscriptions.filter(
        s => !s.amount || s.amount.equals(new Decimal(0))
      ),
      noFrequency: allSubscriptions.filter(s => !s.frequency),
      noNextBilling: allSubscriptions.filter(s => !s.nextBilling),
      notActive: allSubscriptions.filter(
        s => !s.isActive || s.status !== 'active'
      ),
    };

    if (issues.noAmount.length > 0)
      log.warning(`${issues.noAmount.length} subscriptions have no amount`);
    if (issues.noFrequency.length > 0)
      log.warning(
        `${issues.noFrequency.length} subscriptions have no frequency`
      );
    if (issues.noNextBilling.length > 0)
      log.warning(
        `${issues.noNextBilling.length} subscriptions have no next billing date`
      );
    if (issues.notActive.length > 0)
      log.warning(`${issues.notActive.length} subscriptions are not active`);
    ...
    for (const user of users) {
      ...
      console.log(
        `  Active subscriptions found: ${activeSubscriptions.length}`
      );
      ...
      if (activeSubscriptions.length > 0) {
        ...
        for (const sub of activeSubscriptions) {
          console.log(
            `    - ${sub.name}: ${sub.amount} ${sub.currency} (${sub.frequency})`
          );
    ...
    if (allSubscriptions.length === 0) {
      ...
      console.log(
        '  4. Account ID mapping issues preventing transaction saves'
      );
