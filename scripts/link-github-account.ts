...
async function linkGitHubAccount() {
  ...
  if (!emailArg || !githubIdArg) {
    console.error('Usage: npx tsx scripts/link-github-account.ts --email=your@email.com --github-id=YOUR_GITHUB_ID');
  ...
  try {
    ...
    console.log(`Existing OAuth accounts: ${user.accounts.map(a => a.provider).join(', ')}`);
