...
export function OAuthButton({ provider, disabled = false }: OAuthButtonProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (oauthButton) {
        ...
        console.log(`🔐 OAuthButton[${provider}]: Button rect`, rect);
        console.log(`🔐 OAuthButton[${provider}]: Button styles`, {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      console.error(`🔐 OAuthButton[${provider}]: Sign-in error:`, error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const providerConfig = {
    google: {
      className: 'bg-[#4285F4] hover:bg-[#357AE8] focus-visible:ring-[#4285F4]',
  ...
  return (
    <button
      type="button"
      className={`inline-flex h-10 w-full items-center justify-center gap-2 rounded-md px-8 text-sm font-medium text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 ${config.className}`}
      onClick={handleClick}
